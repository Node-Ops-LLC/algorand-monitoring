#!/bin/bash
# Does a multi-step installation of Prometheus, Node Exporter, Grafana, Algorand dashboards
...
# I don't think this is a necessary check, we can add sudo -u <username> to the commands to force it to run in the correct context
# if ((EUID == 0)); then
...
usage () {
  ...
  echo "   --4      Step 4: Installs Algorand Grafana dashboards"
...
$ sudo groupadd --system prometheus # create a prometheus group 
$ sudo useradd -s /sbin/nologin --system -g prometheus prometheus # create the prometheus user and assign it to the prometheus group 
$ sudo mkdir –p /etc/prometheus # create the configuration directory 
$ sudo mkdir –p /var/lib/prometheus # create the data directory 
$ cd /etc/prometheus # navigate to the configuration directory 
# $ sudo –u prometheus wget https://github.com/prometheus/prometheus/releases/download/v2.40.2/prometheus-2.40.2.linux-amd64.tar.gz # download the latest installation file 
# $ sudo –u prometheus tar –xvf prom* # extract the tarball 
# $ cd prometheus-2.40.2.linux-amd64 # navigate to the Prometheus folder 
$ sudo chown -R prometheus:prometheus /etc/prometheus/ /var/lib/prometheus/ # assign directory ownership 
$ sudo chmod -R 775 /etc/prometheus/ /var/lib/prometheus/ # assign directory permissions 
$ sudo –u prometheus mv prometheus promtool /usr/local/bin # move the prometheus and promtool binary files to /usr/local/bin 
$ sudo –u prometheus mv consoles/ console_libraries/ /etc/prometheus/ # move the console files and console_library files to the configuration directory 
$ sudo –u prometheus mv prometheus.yml /etc/prometheus/prometheus.yml # move the .yml config file to the configuration directory 
$ cd ..; ll; # navigate up one directory and list the contents 
$ sudo –u prometheus rm -rf prometheus-2.40.* # remove the tarball and extracted folder 
$ sudo –u prometheus prometheus –version # check the prometheus version 
$ sudo –u prometheus promtool –version # check the promtool version 
$ sudo nano /etc/systemd/system/prometheus.service # make a systemd service 
[Unit] 
...
ExecStart=/usr/local/bin/prometheus \ 
    ...
    --storage.tsdb.retention.size=100GB \ # optional 
    --storage.tsdb.retention.time=120d \ # optional 
...
$ sudo systemctl daemon-reload # reload daemon configuration files (may not be needed) 
$ sudo systemctl start prometheus # start the prometheus systemd service 
$ sudo systemctl enable prometheus # enable the service to run at startup 
$ sudo systemctl status prometheus # confirm the status of the service


install_prometheus() {
  echo "Algorand monitoring installer"
  echo "--------------------------------"
  echo "STEP 1: Installing Prometheus"
  echo
  get_environment
  check_reqs
  mkdir -p /tmp/algorand-monitoring-installer/prometheus
  cd /tmp/algorand-monitoring-installer/prometheus
